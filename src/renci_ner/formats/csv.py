...
class DelimitedFile:
    def __init__(self, filename: str, columns_include: list[str] = None, columns_exclude: list[str] = None, dialect: str = None, gzipped: bool = None):
        ...
        self.columns_include = set(columns_include) if columns_include is not None else set()
        self.columns_exclude = set(columns_exclude) if columns_exclude is not None else set()
    ...
    def read_file(self, include_empty=False, combine_columns=False, root_location=None):
        ...
        if root_location is None:
            self.logger.info(f"Reading {self.filename} as a DelimitedFile with dialect {self.dialect}.")
            root_location = self.filename
        else:
            self.logger.info(f"Reading {self.filename} (root location: {root_location}) as a DelimitedFile with dialect {self.dialect}.")
        count_texts = 0
        count_rows = 0
            ...
            for row in reader:
                ...
                if combine_columns:
                    ...
                    for column in columns_to_include:
                        ...
                        if not include_empty and text.strip() == "":
                            continue
                        yield AnnotatedText(row[column], location=[
                            root_location,
                            self.__class__.__name__,
                            f"row={count_rows}",
                            column,
        ...
        self.logger.info(f"Generated {count_texts} AnnotatedText objects from {count_rows} rows in {self.filename}.")
